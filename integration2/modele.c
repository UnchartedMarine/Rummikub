...
LISTE * separer_liste_en_deux(LISTE *liste, int position){
	...
	//Remplacer la tuile suivante de la derniere tuile de la suite 1 par la tuile vide
	tuileAvant->suivant=NULL;
...
int enleve_element_liste(LISTE * liste,int position){
	...
	if((position > nb_elements_liste(liste)) || (position <= 0) || nb_elements_liste(liste) == 0){
		return 0;
	}
